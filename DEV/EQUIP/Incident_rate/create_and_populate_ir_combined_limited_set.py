...
#   Script Name         : create_and_populate_ir_combined_limited_set.py                                                                #
#   Description         : Full load of ir_cmbnd_vhcl_clm_lmtd_flds                                                                      #
...
hq = SparkSession.builder.appName("ir_cmbnd_vhcl_clm_lmtd_flds").enableHiveSupport().getOrCreate()

hq.sql("DROP TABLE IF EXISTS equip.ir_cmbnd_vhcl_clm_lmtd_flds")

hq.sql("CREATE TABLE equip.ir_cmbnd_vhcl_clm_lmtd_flds  \
STORED AS ORC  \
AS  \
SELECT  \
vhcl.vin_id VIN_ID,  \
...
LEFT OUTER JOIN equip.plant ON \
TRIM(vin_vhcl.mnfctg_vhcl_plnt_cd) = TRIM(plant.vhcl_mnfctr_plnt_cd) \
WHERE  \
