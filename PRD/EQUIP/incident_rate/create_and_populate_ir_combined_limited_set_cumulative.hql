...
DROP TABLE IF EXISTS equip.ir_cmbnd_vhcl_clm_lmtd_flds_cmltv;

CREATE TABLE equip.ir_cmbnd_vhcl_clm_lmtd_flds_cmltv STORED AS ORC AS SELECT 
`^(crte.*|updt.*)?+.+`, '0' AS MIS,
"IR_COMBINED_CMLT_FULL" AS CRTE_USR_ID,
cast(from_unixtime(unix_timestamp()) as timestamp) AS CRTE_TS,
"IR_COMBINED_CMLT_FULL" AS UPDT_USR_ID,
...
cast(from_unixtime(unix_timestamp()) as timestamp) AS UPDT_TS
FROM equip.ir_cmbnd_vhcl_clm_lmtd_flds
WHERE CLAIM_RECORD = 1 AND (CLM_MIS <= '60' OR CLM_MIS is NULL);
